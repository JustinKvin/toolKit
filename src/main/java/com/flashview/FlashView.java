...
import com.flashview.listener.OnItemClickListener;
...
public class FlashView extends FrameLayout {
    ...
    public FlashView(Context context, AttributeSet attrs, int defStyle) {
        ...
        effect = mTypedArray.getInt(R.styleable.FlashView_effect, 0);
    ...
    public void setImageUris(List<String> imageuris) {
        ...
        if (imageuris.size() <= 0)// 如果得到的图片张数为0，则增加一张默认的图片
        {
            ...
            imageUris.add("drawable://" + R.mipmap.loading_fail_icon);
        } else {
            if (imageuris.size() == 2) {
                isTwo = true;
...
//		 }
        ...
        mViewPager.setAdapter(new FlashAdapter());
...
//		 mhandler.sendEmptyMessageDelayed(ImageHandler.MSG_UPDATE_IMAGE,ImageHandler.MSG_DELAY);
    ...
    /**
     * set selected item`s effect
     *
     * @param selectEffect
     */

    public void setItemEffect(int selectEffect) {
        switch (selectEffect) {
            case 0:
